...
function loadAllItem(parentBox,itemWidth,gap){
    ...
    // console.log(originPosition_x);
    // console.log(colum_count);
    // console.log(items[0].clientHeight);
    // console.log(items[0].offsetHeight);
    // console.log(items[0].scrollHeight);
    console.log($(items[0]).height());

    // setTimeout(()=>{
    //     loadItem(parentBox,items,itemWidth,gap);
    // },10)
    // $(".item_containner .item").fadeOut();
    isImgLoad(function(){
        // console.log("图片加载完")
       
        loadItem(parentBox,items,itemWidth,gap);
    },".item_containner .item img")
    // // parentBox.find("img").on("load",function(){
    //     loadItem(parentBox,items,itemWidth,gap);
    // // })
}

function loadItem(parentBox,itemArr,itemWidth,gap){
    //摆放传入的元素
