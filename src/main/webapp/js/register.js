...
function dataIsExiste(field, value, errorMessage) {
    ...
    var result = false;
    $.ajax({
        url : 'http://192.168.137.122:8080/Servlet/IsExistInfoServle',
       data : {
        field: field,
        value: value,
        requestTpye: "get",
...
$('.send_btn').click(function () {
    ...
    if (time == 0 && !isEmpty($('.email_input'), "邮箱禁止为空！") && isEmailAvailable("邮箱有误！")) {
        ...
        //    console.log(requestData.email);
            // console.log('发送验证码');
            $.get("http://192.168.137.122:8080/Servlet/VerifyCodeServlet",{
...
$('.submit_btn').click(() => {
    ...
    $.post('http://192.168.137.122:8080/Servlet/UserServlet', JSON.stringify(formData), function (res) {
