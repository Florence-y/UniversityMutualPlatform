...
var oAuthor;
//整篇文章的id
var questionId_local = "Q11IVnUBiQ9Ajg088h2y";
...
// 窗口缩放
    ...
    function sendAnswer(){
       ...
       //准备发送
        ...
        $.ajax({
            url : "http://192.168.137.108:8080/Servlet/AnswerServlet",
            type : "post",
            dataType : 'json',
    ...
    function loadMyNewAnswer(answerContents,answerId){
        ...
        src = src.replace(reg.exec(src)[0],"http://192.168.137.108:8080//");
        var framData = {
            agreeCount : 0,
            commentCount : 0,
            userType :type,
    ...
    function seeComment(nextPage){
        ...
        if($(this).attr("loadingAbility")=="true"){//请求第一页代表初始化
            // console.log("初始化评论条")
            $.ajax({
                url : "http://192.168.137.108:8080/Servlet/CommentServlet",
                data : {
                    requestType : "get",
                    getType : "answer",
    ...
    function displayComment(dataList){
        for(var i=0; i<dataList.length; i++){
            ...
            var reg = /(..\/)/;
            data.face = data.face.replace(reg.exec(data.face)[0],"http://192.168.137.108:8080//");
    ...
    $('.question_info_main .like_btn .icon').click(function(){
        ...
        if(status=="agree"){//再次点击为取消点赞
            $.ajax({
                url : "http://192.168.137.108:8080/Servlet/AgreeServlet",
                type: "get",
                dataType :"json",
                data : {
                    requestType : "delete",
    ...
    function subscribeAuthor(){
        $.ajax({
            url : "http://192.168.137.108:8080/Servlet/AttentionServlet",
            type : "post",
            dataType : 'json',
            data : JSON.stringify({
...
//获取键和获取值
    ...
    function sendInfo(data) {
        console.log("发送了通知！")
        $.ajax({
            url : "http://192.168.137.108:8080/Servlet/InfServlet",
            type : "post",
