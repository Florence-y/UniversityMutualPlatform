...
$(".modal_bg_lost .objClass .item").on("click",function(){
 ...
 $(".modal_bg_lost .calendar_entrance").on("mouseenter",function(){
    $(this).find(".calendar").show(200);
})
$(".modal_bg_lost .calendar_entrance").on("mouseleave",function(){
    $(this).find(".calendar").hide(200);
})
...
$(".modal_bg_lost .area").on("mouseenter",function(){
    $(this).find(".areaPane").show(200);
});
$(".modal_bg_lost .area").on("mouseleave",function(){
    $(this).find(".areaPane").hide(200);
});
...
function sendImage(formdata, imgObj) { //imgObj是jq对象
    sendingImg = true;
    $.ajax({
        url: 'http://192.168.137.101:8080/Servlet/ReceiveFileServlet',
        type: 'post',
        data: formdata,
        dataType: 'json',
...
function searchFound(content){
    $.ajax({
        url : "http://192.168.137.101:8080//Servlet/LostAndFoundExploreServlet",
        type : "get",
        dataType : "json",
        data : {
            requestType :'get',
...
function displayFound(dataList){
    ...
    for(var i=0; i<dataList.length; i++){
        ...
        if(dataList[i].imgs.length!=0){
            ...
            url = url.replace(reg.exec(url)[0],"http://192.168.137.101:8080//");
            data["img"] = url;
        }
